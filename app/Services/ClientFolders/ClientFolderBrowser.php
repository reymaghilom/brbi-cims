...
class ClientFolderBrowser
{
    ...
    public function __construct(private readonly ClientFolderSavedRecords $savedRecords) {}
    ...
    public function browse(User $user, array $filters): LengthAwarePaginator
    {
        ...
        // Informational only: decides which permanent-delete dialog each rendered folder gets
        // (delete an empty folder / deletion not available). One query for the current page, using
        // the same canonical definition the DELETE endpoint re-checks independently.
        $folders = $paginator->getCollection();
        $withSavedRecords = $folders->isEmpty()
            ? collect()
            : $this->savedRecords->whereHasSavedRecords(ClientFolder::query()->whereKey($folders->modelKeys()))->pluck('id')->flip();
