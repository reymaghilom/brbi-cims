...
// Declared/inspected/not-inspected counts are entered as free text (e.g. "None", "N/A", "3
// units"), not numbers only — every template built on this shared summary block inherits
// that consistently, rather than each overriding its own copy of these fields to type 'text'.
$summary = fn (string $subject): array => [
    $field('total_declared', "Total $subject Declared", 'text'),
    $field('total_inspected', "Total $subject Inspected", 'text'),
    $field('total_not_inspected', "Total $subject Not Inspected", 'text'),
...
// Pharmacy / Drugstore only: # of Shifts and # of Employees per Shift accept free text
// (e.g. "2A", "N/A"), unlike every other template sharing $pharmacyBranchColumns above.
$pharmacyDrugstoreBranchColumns = array_map(static function (array $branchColumn): array {
    return in_array($branchColumn['key'], ['shifts', 'employees_per_shift'], true)
        ? array_replace($branchColumn, ['type' => 'text'])
        : $branchColumn;
}, $pharmacyBranchColumns);
// General Merchandise Store / Hardware / Auto or Motor Parts only: same free-text treatment
// as Pharmacy / Drugstore above, for the same two fields.
$generalMerchandiseBranchColumns = array_map(static function (array $branchColumn): array {
    return in_array($branchColumn['key'], ['shifts', 'employees_per_shift'], true)
        ? array_replace($branchColumn, ['type' => 'text'])
        : $branchColumn;
}, $pharmacyBranchColumns);
// Buy & Sell Store: Dry Goods only: same free-text treatment as above, for the same two fields.
$buySellDryGoodsBranchColumns = array_map(static function (array $branchColumn): array {
    return in_array($branchColumn['key'], ['shifts', 'employees_per_shift'], true)
        ? array_replace($branchColumn, ['type' => 'text'])
        : $branchColumn;
}, $pharmacyBranchColumns);
// Meatshop Store only: same free-text treatment as above, for the same two fields.
$meatshopBranchColumns = array_map(static function (array $branchColumn): array {
    return in_array($branchColumn['key'], ['shifts', 'employees_per_shift'], true)
        ? array_replace($branchColumn, ['type' => 'text'])
        : $branchColumn;
}, $pharmacyBranchColumns);
...
$restaurantBranchColumns = [
    ...
    $column('shifts', '# OF SHIFTS', 'text'),
    $column('employees_per_shift', '# OF EMPLOYEES PER SHIFT', 'text'),
...
return [
    ...
    'distributorship_wholesaler_b2b' => ['schema' => [
        'profile' => true, 'fields' => array_merge([$field('office_staff', '# of Office Staff', 'text'), $field('agents', '# of Agents', 'text'), $field('drivers', '# of Drivers', 'text'), $field('helpers', '# of Helpers', 'text'), $field('inventory_level', 'Inventory Level'), $field('products_seen', 'Products / Goods Seen in Inventory'), $field('top_brands', 'Top Brands Stocked'), $field('top_brand_prices', 'Selling Price')], $summary('Units')),
