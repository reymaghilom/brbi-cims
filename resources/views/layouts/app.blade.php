...
<head>
    ...
    {{-- Must run before any CSS paints: reads the saved desktop sidebar-collapsed preference and
         stamps it on <html> synchronously, so the collapsed layout is correct on first paint
         instead of flashing expanded-then-collapsed. Never applied below the lg breakpoint —
         mobile always keeps the existing off-canvas drawer regardless of this saved preference. --}}
    <script>
        (function () {
            try {
                if (window.matchMedia('(min-width: 1024px)').matches && localStorage.getItem('brbi-sidebar-collapsed') === 'true') {
...
<body class="min-h-screen overflow-x-hidden bg-app-bg text-text-main antialiased">
    ...
    <aside data-mobile-drawer id="primary-sidebar" class="ui-scrollbar fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col overflow-y-auto bg-brand-sidebar text-white shadow-float transition-transform duration-200 lg:translate-x-0 lg:shadow-none" aria-label="Primary navigation">
        <div class="flex min-h-20 items-center justify-between border-b border-white/10 px-5">
            <a href="{{ route('home') }}" class="ui-sidebar-brand min-w-0 flex-1 rounded-control focus-visible:outline-white" aria-label="Binhi Rural Bank Inc. dashboard">
                <img src="{{ asset('assets/branding/binhi-rural-bank-wordmark-light.png') }}" alt="Binhi Rural Bank Inc." class="ui-sidebar-brand-full h-auto w-full max-w-[11.5rem] object-contain object-left">
                <img src="{{ asset('assets/branding/favicon-leaf-128x128.png') }}" alt="Binhi Rural Bank Inc." class="ui-sidebar-brand-compact size-9 object-contain">
            </a>
        ...
        <nav class="flex-1 space-y-6 px-4 py-6">
            <div>
                <p class="ui-sidebar-section-label mb-2 px-3 text-[0.68rem] font-bold uppercase tracking-[0.18em] text-white/45">Workspace</p>
                <div class="space-y-1">
        ...
        <div class="ui-sidebar-footer flex items-center px-4 py-3 text-left">
    ...
    <div id="app-content-wrapper" class="min-h-screen lg:pl-64">
    ...
    <div id="sidebar-tooltip" role="tooltip"></div>
</body>
</html>
