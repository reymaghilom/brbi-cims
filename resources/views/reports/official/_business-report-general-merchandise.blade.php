@php
    ...
    $products = array_pad(array_slice((array) data_get($data, 'tables.products', []), 0, 6), 6, []);
    $suppliers = array_pad(array_slice((array) data_get($data, 'tables.suppliers', []), 0, 3), 3, []);
    $generalMerchandiseQuestions = [
        ...
        'Aside from Walk In Clients: Are there big, recurring customers?',
        'Do they have delivery trucks? (how many)',
        'Are these delivery trucks: owned or mortgaged? (WHICH INST.)',
...
<tr><th colspan="4">TYPE OF STORE (CHECK APPLICABLE):</th><td colspan="21" class="business-options"><div class="business-choice-list">@foreach(['General Merchandise', 'Hardware', 'Auto or Motor Parts', 'Paint Only'] as $option)<span>{{ $mark(strcasecmp($storeType, $option) === 0) }} {{ mb_strtoupper($option) }}</span>@endforeach</div></td></tr>
...
<tbody>
@foreach($generalMerchandiseQuestions as $index => $question)
<tr><td colspan="4">{{ $products[$index]['product'] ?? '' }}</td><td colspan="3" class="business-align-right">{{ $products[$index]['selling_price'] ?? '' }}</td><td colspan="8">{{ ($index + 1).'. '.$question }}</td><td colspan="10">{{ data_get($data, "questions.$index") }}</td></tr>
@endforeach
</tbody></table>
