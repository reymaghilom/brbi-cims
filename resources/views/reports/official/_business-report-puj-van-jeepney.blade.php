...
<thead><tr><th colspan="4">BRAND/CAR MODEL (ONLY THOSE INSPECTED)</th><th colspan="2">YEAR MODEL</th><th colspan="2">PLATE NO.</th><th colspan="2">W/ DRIVER?</th><th colspan="2">SINCE? (MONTH/YEAR)</th><th colspan="2">DAILY BOUNDARY</th><th colspan="3">ROUTE (INDICATED IN THE UNIT)</th><th colspan="2">AVE. FARE FOR RIDER</th><th colspan="6">FRANCHISE/OPERATOR NAME &amp; CONTACT NO. (AS SEEN IN PUJ UNIT)</th></tr></thead>
<tbody>
@foreach($units as $unit)
<tr><td colspan="4">{{ $unit['brand_model'] ?? '' }}</td><td colspan="2">{{ $unit['year_model'] ?? '' }}</td><td colspan="2">{{ $unit['plate_number'] ?? '' }}</td><td colspan="2">{{ $unit['with_driver'] ?? '' }}</td><td colspan="2">{{ $unit['operating_since'] ?? '' }}</td><td colspan="2">{{ $unit['daily_boundary'] ?? '' }}</td><td colspan="3">{{ $unit['route'] ?? '' }}</td><td colspan="2">{{ $unit['average_fare'] ?? '' }}</td><td colspan="6">{{ $unit['franchise_operator'] ?? '' }}</td></tr>
@endforeach
</tbody></table>
