@php
    ...
    $buyers = array_pad(array_slice((array) data_get($data, 'tables.buyers', []), 0, 2), 2, []);
    /**
     ...
     * has-data check) — the same instability as Meatshop's inventory rows. Each fixed
     * category is matched by its saved supplier_category text instead of array position.
     */
...
<tr><th colspan="4" rowspan="2">INDUSTRY RESEARCH/SURVEY:<br>AS OF DATE: {{ $na($researchDate) }}</th><th colspan="4">AVE. SELLING PRICE PER KILO</th><td colspan="3">{{ $na($field('average_selling_price')) }}</td><th colspan="3">SEEDS COST PER HA:</th><td colspan="3">{{ $na($field('seed_cost_per_ha')) }}</td><th colspan="4">FERTILIZER COST PER HA:</th><td colspan="4">{{ $na($field('fertilizer_cost_per_ha')) }}</td></tr>
<tr><th colspan="4">AVE. KILOS YIELD PER HA</th><td colspan="3">{{ $na($field('average_yield_per_ha')) }}</td><th colspan="3">CROP CYCLE TERM (MOS):</th><td colspan="3">{{ $na($field('crop_cycle_months')) }}</td><th colspan="4">PEAK HARVEST MONTHS:</th><td colspan="4">{{ $na($field('peak_harvest_months')) }}</td></tr>
...
<thead><tr><th colspan="6">LOCATION &amp; SIZE OF LAND (HA)</th><th colspan="2">TOTAL HA</th><th colspan="3">OWNED/RENTED/PRENDA</th><th colspan="2">IF RENTED, ANNUAL RENT PER HA</th><th colspan="2">IF PRENDA, AMOUNT PER HA</th><th colspan="2">IF PRENDA/RENT, EXPIRY DATE</th><th colspan="3">TARGET HARVEST MONTH</th><th colspan="5">RELEVANT INFORMATION (EX. IF OWNED - NOT TRANSFERRED, INFO ON PROPERTY OWNER)</th></tr></thead>
<tbody>
@foreach($farms as $farm)
<tr><td colspan="6">{{ $farm['location_size'] ?? '' }}</td><td colspan="2">{{ $farm['total_ha'] ?? '' }}</td><td colspan="3">{{ $farm['tenure'] ?? '' }}</td><td colspan="2">{{ $farm['annual_rent'] ?? '' }}</td><td colspan="2">{{ $farm['prenda_amount'] ?? '' }}</td><td colspan="2">{{ $farm['expiry_date'] ?? '' }}</td><td colspan="3">{{ $farm['target_harvest_month'] ?? '' }}</td><td colspan="5">{{ $farm['relevant_information'] ?? '' }}</td></tr>
@endforeach
</tbody></table>
<table class="business-form-table business-grid-table"><colgroup><col span="25" style="width:4%"></colgroup>
...
<table class="business-form-table business-grid-table"><colgroup><col span="25" style="width:4%"></colgroup>
<thead><tr><th colspan="9">CORN BUYERS:</th><th colspan="5">ADDRESS:</th><th colspan="2">YEARS TRANSACTING</th><th colspan="9">PRODUCTION FERFORMANCE IN LAST HARVEST / OTHER REMARKS:</th></tr></thead>
<tbody>
@foreach($buyers as $buyer)
<tr><td colspan="9">{{ $buyer['buyer'] ?? '' }}</td><td colspan="5">{{ $buyer['address'] ?? '' }}</td><td colspan="2">{{ $buyer['years_transacting'] ?? '' }}</td><td colspan="9">{{ $buyer['production_performance'] ?? '' }}</td></tr>
@endforeach
</tbody></table>
