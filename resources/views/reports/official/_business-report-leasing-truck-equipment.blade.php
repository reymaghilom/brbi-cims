@php
    ...
    $units = array_pad(array_slice((array) data_get($business['template_data'] ?? [], 'tables.units', []), 0, 6), 6, []);
...
<thead><tr><th colspan="5">TYPE OF TRUCK/EQUIPMENT (brand/capacity/fully paid or mortgaged)</th><th colspan="5">RENTED BY (NAME/OFFICE ADDRESS/CONTACT NUMBER)</th><th colspan="4">CURRENT LOCATION (AREA TRAVELLED/GOODS TRANSPORTED)</th><th colspan="2">TOTAL UNITS DECLARED</th><th colspan="2">TOTAL UNITS VALIDATED</th><th colspan="3">RENTAL RATE (php per km/php per cubic/fixed monthly)</th><th colspan="2">W/ CONTRACT? (Y/N)</th><th colspan="2">ORCR CHECKED?</th></tr></thead>
<tbody>
@foreach($units as $unit)
<tr><td colspan="5">{{ $unit['truck_equipment_type'] ?? '' }}</td><td colspan="5">{{ $unit['rented_by'] ?? '' }}</td><td colspan="4">{{ $unit['current_location'] ?? '' }}</td><td colspan="2">{{ $unit['units_declared'] ?? '' }}</td><td colspan="2">{{ $unit['units_validated'] ?? '' }}</td><td colspan="3">{{ $unit['rental_rate'] ?? '' }}</td><td colspan="2">{{ $unit['has_contract'] ?? '' }}</td><td colspan="2"></td></tr>
@endforeach
</tbody></table>
<table class="business-form-table business-grid-table"><colgroup><col span="25" style="width:4%"></colgroup>
<thead><tr><th colspan="4">EMPLOYEES</th><th colspan="4">SUPPLIERS</th><th colspan="5">BUSINESS NAME/CONTACT PERSON</th><th colspan="5">ADDRESS</th><th colspan="3">YEARS TRANSACTING</th><th colspan="4">MONTHLY AVE EXPENSE &amp; PAYMENT TRACK RECORD</th></tr></thead>
<tbody>
...
<tr><td colspan="2">{{ $label }}</td><td colspan="2">{{ $na($field($key)) }}</td><td colspan="4">{{ $supplier['supplier_category'] ?? $supplierCategories[$index] }}</td><td colspan="5">{{ $supplier['supplier_name'] ?? '' }}</td><td colspan="5">{{ $supplier['office_location'] ?? '' }}</td><td colspan="3">{{ $supplier['years_transacting'] ?? '' }}</td><td colspan="4">{{ $supplier['payment_performance'] ?? '' }}</td></tr>
@endforeach
</tbody></table>
