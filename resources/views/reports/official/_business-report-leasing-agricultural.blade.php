@php
    ...
    $properties = array_pad(array_slice((array) data_get($business['template_data'] ?? [], 'tables.properties', []), 0, 6), 6, []);
@endphp
...
<tr><th colspan="6">TOTAL PROPERTIES DECLARED:</th><td colspan="3">{{ $na($field('total_declared')) }}</td><th colspan="6">TOTAL PROPERTIES INSPECTED:</th><td colspan="2">{{ $na($field('total_inspected')) }}</td><th colspan="4">NOT INSPECTED:</th><td colspan="4">{{ $na($field('total_not_inspected')) }}</td></tr>
<tr><th colspan="6">REASON NOT INSPECTED:</th><td colspan="19">{{ $na($field('reason_not_inspected')) }}</td></tr>
</tbody></table>
...
<thead><tr><th colspan="5">LOCATION &amp; TOTAL HA</th><th colspan="4">TENANT (NAME &amp; YEARS RENTING)</th><th colspan="3">ANNUAL RENT</th><th colspan="2">W/ CONTRACT? (Y/N)</th><th colspan="3">PLANTED CROP</th><th colspan="3">CONTACT NO. OF TENANT</th><th colspan="5">RELEVANT INFORMATION (EX. LEASE INCOME SHARED AMONG RELATIVES?)</th></tr></thead>
<tbody>
@foreach($properties as $property)
<tr><td colspan="5">{{ $property['location_area'] ?? '' }}</td><td colspan="4">{{ $property['tenant'] ?? '' }}</td><td colspan="3">{{ $property['annual_rent'] ?? '' }}</td><td colspan="2">{{ $property['has_contract'] ?? '' }}</td><td colspan="3">{{ $property['planted_crop'] ?? '' }}</td><td colspan="3">{{ $property['tenant_contact'] ?? '' }}</td><td colspan="5">{{ $property['relevant_information'] ?? '' }}</td></tr>
@endforeach
</tbody></table>
