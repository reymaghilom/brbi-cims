@php
    ...
    $products = array_pad(array_slice((array) data_get($data, 'tables.products', []), 0, 4), 4, []);
...
<table class="business-form-table business-grid-table"><colgroup><col span="25" style="width:4%"></colgroup>
<thead><tr><th colspan="4">Top Sellable Products (branded or generic):</th><th colspan="3">Selling Price per Item</th><th colspan="18">OBSERVATIONS DURING BUSINESS INSPECTION:</th></tr></thead>
<tbody>
@foreach($pharmacyQuestions as $index => $question)
<tr><td colspan="4">{{ $products[$index]['product'] ?? '' }}</td><td colspan="3" class="business-align-right">{{ $products[$index]['selling_price'] ?? '' }}</td><td colspan="8">{{ ($index + 1).'. '.$question }}</td><td colspan="10" @class(['business-align-left' => $index === 0])>{{ data_get($data, "questions.$index") }}</td></tr>
@endforeach
</tbody></table>
