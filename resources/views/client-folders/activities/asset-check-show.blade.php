            ...
            <div class="mt-4 grid gap-3 lg:grid-cols-2" data-asset-target-list>
                @forelse($activity->assetTargets as $target)
                    ...
                    <article class="min-w-0 rounded-card border border-ui-border bg-surface p-4 shadow-sm" data-asset-target-card="{{ $target->id }}">
                        <div class="flex items-start gap-3"><input type="checkbox" class="mt-1 size-4 shrink-0 rounded border-ui-border text-success focus:ring-success" aria-label="{{ $targetCompleted ? $targetLabel.' is completed' : 'Mark '.$targetLabel.' as completed' }}" @checked($targetCompleted) @disabled($targetCompleted) @if(! $targetCompleted) data-asset-target-complete data-asset-modal-open="complete-asset-target-{{ $target->id }}" @endif><div class="min-w-0 flex-1"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><h3 class="break-words text-base font-bold text-brand-sidebar">{{ $target->assessorLabel() }}</h3><p class="mt-0.5 break-words text-sm text-text-muted">{{ $target->office_location }}</p></div><x-ui.status-badge :status="$target->status" /></div></div></div>
                        ...
                        <p class="mt-3 break-words text-xs text-text-muted">Updated by {{ $target->updater?->full_name ?? 'System' }} · {{ $target->updated_at->timezone(config('cims.display_timezone'))->format('M j, Y g:i A') }}</p>
                        <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-ui-border pt-3"><button type="button" class="ui-button-secondary-compact" data-asset-modal-open="edit-asset-target-{{ $target->id }}"><x-ui.icon name="edit" size="size-3.5" />Edit</button><button type="button" class="ui-button-danger-compact" data-asset-modal-open="delete-asset-target-{{ $target->id }}"><x-ui.icon name="trash" size="size-3.5" />Delete</button></div>
                    </article>
                @empty
