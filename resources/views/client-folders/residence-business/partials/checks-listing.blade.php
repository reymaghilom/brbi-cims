{{-- The Residence & Business Report page's whole refreshable listing area.

     Rendered inline by residence-business/edit.blade.php on a normal page load, and returned on
     its own by ResidenceBusinessReportController::edit() for an XHR fragment request — the exact
     same authoritative view data either way. That is what lets a confirmed Residence/Business
     Check save AUTO-UPDATE the tables in place (app.js's refreshChecksListing) instead of
     reloading the whole page around them.

     Everything that must stay in sync with the rows lives INSIDE [data-checks-listing]: both
     tables, their empty states, the combined selection toolbar (its "of N selected" total moves
     with the row count) and every per-row photo/delete <dialog> (their ids carry row ids). The
     page-level batch <form>s and the shared check-report-dialog deliberately stay outside — they
     are context-only and must survive the swap. --}}
<div data-checks-listing>
    @php
    ...
    @foreach($businessChecks as $check)
        ...
        <x-ui.confirmation-dialog id="delete-business-check-{{ $check->id }}" title="Permanently Delete Business Check?" :action="route('client-folders.business-checks.destroy', [$clientFolder, $check] + $personParams)" method="DELETE" confirm-label="Delete Permanently" cancel-icon="close" confirm-icon="trash" destructive>
