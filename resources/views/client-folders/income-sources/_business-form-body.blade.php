...
@if(in_array('branches', $tags, true))
    @php
        $branchFields = $template->template_type === 'retail_grocery_water_refilling' ? [
            ['name' => 'location', 'label' => 'LOCATION'], ['name' => 'frontage_meters', 'label' => 'FRONT (SQM)'], ['name' => 'total_area_square_meters', 'label' => 'TOTAL SQM'], ['name' => 'is_air_conditioned', 'label' => 'AIRCON (Y/N)', 'format' => 'yes_no'], ['name' => 'operating_days_hours', 'label' => 'OPERATING DAYS & HOURS'], ['name' => 'shifts_count', 'label' => '# OF SHIFTS', 'type' => 'text'], ['name' => 'employees_per_shift', 'label' => '# OF EMPLOYEES PER SHIFT', 'type' => 'text'], ['name' => 'average_sales_per_shift', 'label' => 'AVE. PHP SALES PER SHIFT'], ['name' => 'inventory_level', 'label' => 'INVENTORY LEVEL (HIGH, MID, LOW)'], ['name' => 'monthly_rent', 'label' => 'RENT PER MONTH'], ['name' => 'years_in_area', 'label' => 'YEARS IN THE AREA'], ['name' => 'nearby_brands', 'label' => 'BIG BRANDS NEAR THE AREA?'],
        ] : [
    ...
    @endphp
    @include('client-folders.income-sources._business-repeater', ['section' => 'branches', 'title' => 'Summary of Branches Inspected', 'records' => $report?->branches ?? collect(), 'fields' => $branchFields, 'defaultRows' => $template->template_type === 'retail_grocery_water_refilling' ? 3 : 1, 'tableClass' => $template->template_type === 'retail_grocery_water_refilling' ? 'business-retail-branches-table' : null])
@endif
