@props(['name', 'folderNumber' => null, 'status' => 'on_progress', 'progress' => 0, 'assignedCi' => null, 'createdAt' => null, 'updatedAt' => null])
...
<section {{ $attributes->class('ui-panel p-3.5 sm:p-4') }}>
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="flex min-w-0 items-center gap-3.5">
            ...
            <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">@if(filled($folderNumber))<p class="text-xs font-medium text-text-muted">{{ $folderNumber }}</p>@endif<x-ui.status-badge :status="$status" /></div>
