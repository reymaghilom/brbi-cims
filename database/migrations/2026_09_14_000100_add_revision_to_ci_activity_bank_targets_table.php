...
/**
 * Monotonic edit token for optimistic concurrency on a single Bank / Coop target, mirroring the
 * column residence_checks, business_checks and ci_activities already carry.
 *
 ...
 * the same target simply ran one after the other and the later form silently overwrote the earlier
 * save. updated_at cannot fill that gap either — it is second-precision, so two saves landing in
 * the same second share an identical token.
 *
